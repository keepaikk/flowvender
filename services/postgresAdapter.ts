...
export const getPostgresConfig = () => {
  return localStorage.getItem('flow_postgres_url') || CONFIG.API_BASE || 'http://localhost:3000/api';
};
...
export const getPostgresAdapter = (): DatabaseAdapter => {
  log('Connected to PostgreSQL (demo mode)');

  const api = {
    getProducts: async (): Promise<Product[]> => {
      log(`PostgreSQL GET ${API_BASE}/products`);
      try {
    ...
    getOrders: async (): Promise<Order[]> => {
      log(`PostgreSQL GET ${API_BASE}/orders`);
      try {
        // Production endpoint structure:
    ...
    saveOrder: async (order: Order): Promise<void> => {
      log(`PostgreSQL POST ${API_BASE}/orders`, order);
      try {
        // Production endpoint structure:
        // const response = await fetch(`${API_BASE}/orders`, {
        //   method: 'POST',
        ...
        log('Order saved successfully (demo)');
      } catch (e) {
        log('Failed to save order:', e);
        throw e;
      }
    },

    searchProducts: async (query: string): Promise<Product[]> => {
      log(`PostgreSQL GET ${API_BASE}/products/search?q=${query}`);
      try {
        // Production endpoint structure:
    ...
    getFlashDeals: async (): Promise<Product[]> => {
      log(`PostgreSQL GET ${API_BASE}/products/flash-deals`);
      try {
        // Production endpoint structure:
        ...
        log('Failed to fetch flash deals:', e);
        return [];
      }
    },

    getMadeInGhana: async (): Promise<Product[]> => {
      log(`PostgreSQL GET ${API_BASE}/products/made-in-ghana`);
      try {
        // Production endpoint structure:
        ...
        log('Failed to fetch Ghana products:', e);
        return [];
      }
    },

    getWishlistProducts: async (): Promise<Product[]> => {
      log(`PostgreSQL GET ${API_BASE}/wishlist`);
      try {
        // Production endpoint structure:
    ...
    isInWishlist: async (productId: string): Promise<boolean> => {
      log(`PostgreSQL GET ${API_BASE}/wishlist/${productId}`);
      try {
        // Production endpoint structure:
...
log('PostgreSQL adapter initialized');
